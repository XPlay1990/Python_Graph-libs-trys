...
class Visualizer(object):
    def __init__(self):
        ...
        self.w.opts['distance'] = 40                                    #zoom
        self.w.setWindowTitle('PAS 3D Example')
        self.w.setGeometry(0, 110, 1920, 1080)                          #Randabstände/fensterauflösung
        self.w.show()
    ...
    def update(self):
        for i in range(self.n):
            ...
            z = []
            for t in range(1000):
                z.append(randint(0,9)) #10 * np.cos(d + self.phase) / (d + 1)
            pts = np.vstack([self.x, yi, z]).transpose()
            self.set_plotdata(
                name=i, points=pts,																#data
                color=pg.glColor((i, self.n * 1.3)),											#farbe
                width=(i + 1) / 10 																#linienstärke
            )
            self.phase -= .003
